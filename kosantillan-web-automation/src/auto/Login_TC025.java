...
public class Login_TC025 {
	...
	@Test
	public static void LOGIN_TC025_Reset_Subuser_password_through_Subuser_account(@Optional("chrome") String browser, @Optional("SG") String activesite, @Optional("Test") String activeenvironment) throws Exception {

		Constant.browser = browser;	
		logger = Reporter.PrepareReport(activesite+" LOGIN TC025 Reset Subuser password through Subuser account");

		LaunchBrowser.Execute(browser, activesite, activeenvironment, false, true);
		
		ConfigReader config = new ConfigReader();
		String Language = config.getLanguage();
		Login.SubUser(browser, Language, false, activesite);
		
		//Get Sub User Password based on activesite
		String SubPassword = DoThis.getSubPassword(activesite);
			
		Dictionary ChangePasswordDetails = new Hashtable();
		ChangePasswordDetails.put("current_password", SubPassword);
